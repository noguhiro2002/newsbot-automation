# Phase 2 Broad Lane: PR・ニュースワイヤー

指定期間 `{{PERIOD}}` の国内外のPR・ニュースワイヤー・公式発表から、Lab Automationまたは科学実験workflowに直接関係する重要ニュースを独立した自然文検索で探してください。

structuredの検索語、企業watchlist、候補、feed候補は受け取っていません。固定語に依存しない検索を最低3件実行してください。PRを入口にしても発表主体の公式URLを探し、見つからない場合だけPR URLをcanonicalにしてください。論文・preprintは除外します。

Audience/editorial feedback is a ranking aid, never a hard exclusion:
{{PREFERENCE_PROFILE}}

イベント日を公開日に代入せず、単なる出展・一般セミナーは除外してください。
...
`search_coverage.broad_queries_run`は検索回数を表す数値ではありません。実行した検索文そのものを格納するJSON文字列配列で、空文字を含めず最低3要素にしてください。`19`や`{"count": 19}`のような形式は禁止です。検索回数を別途示したい場合は`broad_query_count`へ数値で入れてください。
